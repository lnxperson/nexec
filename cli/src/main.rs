...
#[derive(Parser)]
#[command(name = "nexec", version, about = "nexec boot manager installer and management tool")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Subcommand)]
enum Commands {
    /// Install nexec to the ESP and register with firmware
    Install {
        ...
        #[arg(long)]
        part: Option<u32>,
        /// Path to a pre-built nexec-efi.efi (builds from source if not specified)
        #[arg(long)]
        efi: Option<String>,
        ...
        /// Skip config file creation/overwrite (used by nexec update)
        #[arg(long)]
        no_config: bool,
    },
    ...
    Status,
    /// Remove nexec from the ESP and UEFI boot entries
    Remove {
        /// Path to the ESP mount point (auto-detected if not specified)
        #[arg(long)]
        esp: Option<String>,
        ...
        #[arg(long)]
        all: bool,
        /// Also remove the nexec CLI binary (/usr/bin/nexec)
        #[arg(long)]
        self_remove: bool,
    },
    /// Update nexec to the latest release
    Update,
    /// Manage boot entries
    Entry {
        #[command(subcommand)]
...
enum ConfigAction {
    /// Generate a sample nexec.conf
        Init {
        /// Output path (default: ./nexec.conf)

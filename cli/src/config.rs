...
pub fn init(output: String) {
    ...
    // Write main config
    let main_content = r#"# nexec main configuration
# Place this file at /EFI/nexec/nexec.conf on your ESP.
...
pub fn generate_detected_config(esp: &str) -> (String, Vec<(String, String)>) {
    ...
    if let Some((os_id, os_name)) = current_os() {
        ...
        let (efi_path, initrd_list) = if let Some(ref path) = kernel_on_esp {
            ...
            eprintln!("  nexec cannot boot without a kernel. Run 'sudo nexec config edit' to configure manually.");
            (String::new(), Vec::new())
        };
    ...
    main_conf.push_str("# nexec main configuration\n");
...
pub fn set_default(entry: String, esp_path: Option<String>) {
    ...
    if !found {
        ...
        let entry_path = format!("{}/EFI/nexec/entries/{}.conf", esp, entry);
        if !Path::new(&entry_path).exists() {
            // Also check old-style (entry might not have a file yet)
            eprintln!("warning: entry '{}' not found in {}/EFI/nexec/entries/", entry, esp);

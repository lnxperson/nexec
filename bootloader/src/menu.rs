...
pub fn show_status(lines: &[(&str, Color)]) {
    uefi::system::with_stdout(|g| {
        let (cols, rows) = g.current_mode().ok().flatten()
            ...
            .unwrap_or((80, 25));
        let title = format!("nexec v{}", VERSION);
...
fn draw_no_entries(keybinds: &Keybinds) {
    uefi::system::with_stdout(|g| {
        ...
        draw_empty(g, w, start_x);
        draw_line(g, &format!("{}  manual boot", keybinds.manual), w, Color::DarkGray, start_x);
        draw_line(g, &format!("{}  firmware setup", keybinds.firmware), w, Color::DarkGray, start_x);
        draw_empty(g, w, start_x);
        draw_bottom(g, w, start_x);
        set_fg_bg(g, Color::Black, Color::Black);
        indent(g, start_x);
...
fn handle_key(key: Key, menu: &mut Menu) -> KeyAction {
    ...
    match key {
        ...
        Key::Printable(c) => {
            let c_val: u16 = c.into();
            // Enter/Return always boots
            if (c_val == b'\r' as u16 || c_val == b'\n' as u16) && !menu.entries.is_empty() {
                return KeyAction::Boot;
            }
            let ch = match core::char::from_u32(c_val as u32) {
                Some(c) => c.to_ascii_lowercase(),
                None => return KeyAction::Nothing,
            };
            ...
            // 1-9 direct entry selection (always active)

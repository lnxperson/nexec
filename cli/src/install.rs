...
const EFI_FILENAME: &str = "nexec-efi.efi";
const EFI_INSTALL_DIR: &str = "/EFI/nexec/nexec-efi.efi";
const CLI_INSTALL_PATH: &str = "/usr/bin/nexec";
...
pub fn install(args: InstallArgs) {
    ...
    let install_dir = format!("{}/EFI/nexec", esp.trim_end_matches('/'));
    ...
    // 5. Install nexec CLI to system path
    ...
    if !no_config {
        ...
        let esp_root_conf = format!("{}/nexec.conf", esp.trim_end_matches('/'));
    ...
    if !register_efibootmgr(&resolved_disk, resolved_part, EFI_INSTALL_DIR) {
        ...
        if let Err(e) = std::fs::copy(&install_path, &fallback_path) {
            ...
            cprintln!(YELLOW, "  Your firmware will boot nexec automatically via the UEFI fallback path.");
            cprintln!(YELLOW, "  (No 'nexec' entry in boot menu — it just boots directly.)");
        }
    }

    println!();
    cprintln!(BOLD_GREEN, "nexec installed successfully!");
    if secure_boot_active() {
        cprintln!(GREEN, "  Secure Boot is enabled — the bootloader is signed.");
    }
    cprintln!(YELLOW, "Reboot and select 'nexec' from your UEFI boot menu.");
}
...
fn download_efi_from_release() -> Option<PathBuf> {
    let tmp = std::env::temp_dir().join("nexec_efi_download");
...
fn register_efibootmgr(disk: &str, part: u32, loader_path: &str) -> bool {
    ...
    // Check if nexec is already registered
    if let Ok(output) = Command::new("efibootmgr").output() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        for line in stdout.lines() {
            if line.contains("nexec") {
                ...
                {
                    cprintln!(YELLOW, "  nexec already registered at Boot{:04X}, skipping.", boot_num);
                    return true;
                }
            }
        }
    ...
    let cmd_str = format!(
        "efibootmgr --create --disk {} --part {} --label \"nexec\" --loader \"{}\"",
...
pub fn status() {
    ...
    match esp {
        Some(ref path) => {
            let installed = format!("{}/EFI/nexec/{}", path.trim_end_matches('/'), EFI_FILENAME);
            if Path::new(&installed).exists() {
                ...
                cprintln!(DIM, "  Run 'nexec install' to install.");
            }
        }
        None => {
            cprintln!(YELLOW, "  ESP: not detected");
            cprintln!(DIM, "  Run 'nexec install --esp <path>' to specify manually.");
        }
    }
}
...
pub fn remove(esp_path: Option<String>, no_efi: bool, all: bool, remove_self: bool) {
    ...
    if all {
        let config_paths = [
            format!("{}/nexec.conf", esp.trim_end_matches('/')),
            format!("{}/EFI/nexec/nexec.conf", esp.trim_end_matches('/')),
        ];
        ...
        let entries_dir = format!("{}/EFI/nexec/entries", esp.trim_end_matches('/'));
    ...
    println!();
    if removed_anything {
        cprintln!(BOLD_GREEN, "nexec removed successfully.");
    }
}
